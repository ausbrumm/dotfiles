...
-- Prefer rzls (Microsoft's official Roslyn LS executable)
local rzls_path = mason .. "/rzls"
local roslyn_path = mason .. "/roslyn"
...
vim.lsp.config("roslyn", {
	cmd = {
		...
		vim.fn.stdpath("state") .. "/lsp-roslyn",
	},
...
-- rzls.nvim for Razor files
require("rzls").setup({
	path = mason .. "/rzls",

	-- For C# support inside Razor, rzls needs roslyn
	roslyn = {
		path = mason .. "/roslyn",
	},
	...
	on_attach = function(client, bufnr)
		...
		vim.keymap.set("n", "<leader>cf", function()
			vim.lsp.buf.code_action({
				filter = function(action)
					return action.kind
						and (action.kind:match("source.organizeImports") or action.title:match("Organize"))
				end,
				apply = true,
			})
		end, opts)
